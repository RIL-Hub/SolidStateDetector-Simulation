#!/usr/bin/env julia
"""
Export electron and hole drift paths (position vs time) for a small set
of representative events. Used to visualize how the charge cloud
migrates through the detector in the (x, z) plane.

Reads:  ENV["GEOMETRY"] (default = czt_cross_strip_full.yaml)
Writes: ENV["OUTPUT"]   (default = output/lateral/drift_paths_export.json)

Simulates 5 x-positions × 3 depths = 15 events at V=−80V (default
geometry), 200 µm cloud, 50 carriers × 2 shells = 100 carriers per
event, each with its own drift path.
"""
...
const OUT_JSON = get(ENV, "OUTPUT",
    joinpath(REPO, "output", "lateral", "drift_paths_export.json"))

const REFINE = [0.2, 0.1, 0.05]
const X_LIST = [-0.4, -0.2, 0.0, +0.2, +0.4]
const Z_LIST = [2.0, 0.0, -2.0]    # SSD z; = paper depths 0.5, 2.5, 4.5 mm
const Y_MM = 2.5
...
# NB: no weighting-potential solve needed — we only need drift paths,
# not induced signals
...
# ── Extract drift paths for one event ──
function extract_paths(evt)
    """Return array of per-carrier dicts with e_path and h_path
    (each: xs, zs vector) and timestamps."""
    dps = evt.drift_paths
    n_carriers = length(dps)
...
for z_mm in Z_LIST
    ...
    for x_mm in X_LIST
        print("  x=$(x_mm) mm, z=$(z_mm) mm (depth $(depth_from_anode) mm) … "); flush(stdout)
        ...
        println("$(round(t;digits=1))s   ($(length(evt.drift_paths)) drift paths)")
    end
end
