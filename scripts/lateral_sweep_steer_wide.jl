#!/usr/bin/env julia
"""
Wide lateral scan (−2 to +2 mm across A17–A23) at mid-depth (z_ssd = 0).

x range: −2.0 to +2.0 mm, 50 µm step (81 points).
Depth:   SSD z = 0 → 2.5 mm from anode.
Capture: A17…A23 (7 anode strips) + steering electrode.
...
const CAPTURE_ANODES = [17, 18, 19, 20, 21, 22, 23]   # widened to 7 strips
const STEER_ID = 40
const N_ANODE = 39
...
println("Lateral scan at depth $DEPTH_MM mm (mid-crystal), "
        * "$(length(X_RANGE)) x-points from $(minimum(X_RANGE)) to "
        * "$(maximum(X_RANGE)) mm:")
for x in X_RANGE
